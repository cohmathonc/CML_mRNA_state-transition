...
D_inc1 = 0.02;
D_tki = 0.072635;
G1 = @(t,X) sqrt( 2*(D_tki +(D_inc1 - D_tki)*(heaviside(t-ti(1))-heaviside(t-ti(2))) ));
F1 = @(t,X) -LUp(X,fCML) + gamma_inc1*dtki*exp(-lam1*(t-ti(2))*heaviside(t-ti(2)))*(heaviside(t-ti(1)))*LUp(X,fCML); 
langevin1 = sde(F1, G1,'StartState',c1);
[Si1,T1] = simulate(langevin1, nPeriods, 'DeltaTime', dt, 'nTrials', Ntrials);
Si1 = Si1(:,all(~isnan(Si1)));
Si1 = Si1(:,sum(Si1 <= 1.4 & Si1 > -0.4) == size(Si1,1));
[ds,ixs] = min(abs(T1-6));
[de,ixe] = min(abs(T1-9));
Si1 = Si1(:,Si1(ixs,:) < (c4 + c5)/2);

% find trajectories reached c1
dtki_p_i = [dtki_p_i size(find(sum(Si1(ixs:end,:) >= c2) > 0 == 1),2)/size(Si1,2)];

end
dtki_p = [dtki_p;dtki_p_i];
end
figure(11);
errorbar(dtki_range,mean(dtki_p),std(dtki_p))
%plot(dtki_range,dtki_p_i)
...
D_inc2 = 0.002;
D_toto = 0.087572573;
G2 = @(t,X) sqrt( 2*(D_toto +(D_inc2 - D_toto)*heaviside(t-ti(1)) ) );
F2 = @(t,X) -LUp(X,fCML) + gamma_inc2*(heaviside(t-ti(1)))*LUp(X,fCML);
langevin2 = sde(F2, G2,'StartState',c1);
[Si2,T2] = simulate(langevin2, nPeriods, 'DeltaTime', dt, 'nTrials', Ntrials);
Si2 = Si2(:,all(~isnan(Si2)));
Si2 = Si2(:,sum(Si2 <= 1.4 & Si2 > -0.4) == size(Si2,1));
Si2 = Si2(:,Si2(ixs,:) < (c4 + c5)/2);

figure(101);
clf;
plot(T1,Si1,'Color',[.8 .8 .8]); hold on;
plot(T2,Si2,'Color',[.4 .4 .4]); hold on;
